...
CREATE INDEX term_idx ON dictionaries USING hash (term);
...
psql -c "DROP TABLE IF EXISTS auto_lang_mblog;CREATE TABLE auto_lang_mblog AS SELECT DISTINCT ON (tweet_id) tweet_id,counting_mblog.lang, probsum as scoreMB from counting_mblog, task1, microblog WHERE task1.id_original=microblog.id_original AND microblog.id=counting_mblog.tweet_id ORDER BY tweet_id ASC, probsum DESC ;"
psql -c "DROP TABLE IF EXISTS auto_lang_wiki;CREATE TABLE auto_lang_wiki AS SELECT DISTINCT ON (tweet_id) tweet_id,counting_wiki.lang,probsum as scorewiki from counting_wiki, task1, microblog WHERE task1.id_original=microblog.id_original AND microblog.id=counting_wiki.tweet_id ORDER BY tweet_id ASC, probsum DESC ;"
